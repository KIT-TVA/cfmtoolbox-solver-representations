...
from cfmtoolbox_smt_encoder import run_smt_solver_with_cloning_base_gap_detection, \
    ...
    run_smt_solver_with_multisetencoding_gap_detection, \
    run_smt_solver_with_multisetencoding_maximize_cardinalities, \
    run_smt_solver_with_multisetencoding_minimize_cardinalities


class TestCloningSMT(unittest.TestCase):

   def test_gaps_evalModel7_multiset_basic(self):
       file = open("../cfms/model_7.json", "rb")
       self.cfm = import_json(file.read())
       file.close()
       result = run_smt_solver_with_multisetencoding_maximize_cardinalities(cfm=self.cfm)
       print(result)


   def test_gaps_multiplayer_multiset(self):
       file = open("../cfms/multiplayer.json", "rb")
       self.cfm = import_json(file.read())
       file.close()

       result = run_smt_solver_with_multisetencoding_gap_detection(cfm=self.cfm)
   ...
   def test_maximize_cardinalities_multiplayer_multiset(self):
       file = open("../cfms/multiplayer.json", "rb")
       self.cfm = import_json(file.read())
       file.close()
   ...
   def test_minimize_cardinalities_multiplayer_multiset(self):
       file = open("../cfms/multiplayer.json", "rb")
       self.cfm = import_json(file.read())
       file.close()

       result = run_smt_solver_with_multisetencoding_minimize_cardinalities(cfm=self.cfm)
